...
export class Distribution extends Construct {
  ...
  constructor(scope: Construct, id: string, props: DistributionProps) {
    ...
    this.distribution = new cdk.aws_cloudfront.Distribution(this, "Distribution", {
      ...
      defaultBehavior: {
        origin: new cdk.aws_cloudfront_origins.S3Origin(props.frontend.bucket, {
    ...
    this.distribution.addBehavior("/api/v1/*", 
      new cdk.aws_cloudfront_origins.HttpOrigin(props.backend.restApiEndpoint), {
        cachePolicy: cdk.aws_cloudfront.CachePolicy.CACHING_DISABLED,
        viewerProtocolPolicy: cdk.aws_cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,
        responseHeadersPolicy: cdk.aws_cloudfront.ResponseHeadersPolicy.CORS_ALLOW_ALL_ORIGINS,
        allowedMethods: cdk.aws_cloudfront.AllowedMethods.ALLOW_ALL,
        originRequestPolicy: cdk.aws_cloudfront.OriginRequestPolicy.CORS_CUSTOM_ORIGIN,
      });    

    props.frontend.bucket.addToResourcePolicy( new cdk.aws_iam.PolicyStatement({
